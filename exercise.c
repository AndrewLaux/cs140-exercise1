 ...
 * In arg: 		size : array size.  It has to be positive.
 * Return SUCC (namely 1) if successful, otherwise FAIL (namely 0)
 *
...
int reverse_array(int a[], int size) {
  ...
  if(size < 1) return FAIL;
	int *temp;
  temp = (int *)malloc(sizeof(int)*size);
  memcpy(temp, a, sizeof(int)*size);
  for(int i = 0; i < size; i++) a[i] = temp[size - 1 - i];
  free(temp);
	return SUCC;
}
/*-------------------------------------------------------------------
 ...
 * 		arg : input number  
 * Return: the result of addition. If there is no match, return the orignal value of input number arg.
 * Example:  match_add("add2", 3) returns 5.
 */
int match_add(char *cmd, int arg){
  if(!cmd) return arg;
	if(strcmp("add1", cmd) == 0) return add1(arg);
	else if(strcmp("add2", cmd) == 0) return add2(arg);
	else if(strcmp("add3", cmd)== 0) return add3(arg);
	else return arg;
}
